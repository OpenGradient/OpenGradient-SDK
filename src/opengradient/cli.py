...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Enable the provider's native web search (billed per search). Supported by OpenAI, Anthropic, Google, and xAI models.",
)
@click.option(
...
@click.option(
    ...
    default=None,
    help="Directory to write images generated by image-output models (e.g. google/gemini-3.1-flash-image). Defaults to the current directory.",
)
@click.option(
    "--x402-settlement-mode",
    type=click.Choice(x402SettlementModes.keys()),
...
def chat(
    ...
    try:
        ...
        if stream:
            print_streaming_chat_result(model_cid, result, is_tee=True, image_output_dir=image_output_dir)
        else:
...
def _save_generated_images(images, output_dir=None):
    """Decode generated image ``data:`` URIs and write them to disk.

    Returns the list of file paths written. Images that cannot be decoded are skipped.
    """
    if not images:
        return []
    ...
    prefix = f"og_image_{int(time.time())}"
    saved_paths = []
    for index, image in enumerate(images):
        match = _DATA_URI_RE.match(image)
        if match:
            mime = (match.group("mime") or "image/png").strip()
            payload = match.group("data")
        else:
            # Not a data URI; assume raw base64 PNG.
            mime = "image/png"
            payload = image
        try:
            raw = base64.b64decode(payload)
        except Exception:
            click.secho(f"  Could not decode generated image #{index + 1}", fg="red")
            continue
        ext = _MIME_EXTENSIONS.get(mime, "png")
        suffix = f"_{index + 1}" if len(images) > 1 else ""
        path = target_dir / f"{prefix}{suffix}.{ext}"
        path.write_bytes(raw)
        saved_paths.append(path)
    return saved_paths


def _print_generated_images(images, output_dir=None):
    """Save any generated images to disk and report their paths."""
    if not images:
        return
    click.secho(f"Generated {len(images)} image(s):", fg="yellow", bold=True)
    saved_paths = _save_generated_images(images, output_dir)
...
def print_llm_chat_result(model_cid, tx_hash, finish_reason, chat_output, is_vanilla=True, result=None, image_output_dir=None):
    ...
    for key, value in chat_output.items():
        if value is None or value in ("", "[]", []):
            continue
        if key == "images":
            # Rendered separately below to avoid dumping base64 data URIs.
            continue
        if key == "tool_calls":
    ...
    images = result.images if result is not None else chat_output.get("images")
    _print_generated_images(images, image_output_dir)


def print_streaming_chat_result(model_cid, stream, is_tee=True, image_output_dir=None):
    """Handle streaming chat response with typed chunks - prints in real-time"""
    asyncio.run(_print_streaming_chat_result_async(model_cid, stream, is_tee, image_output_dir))


async def _print_streaming_chat_result_async(model_cid, stream, is_tee=True, image_output_dir=None):
    ...
    try:
        ...
        async for chunk in stream:
            ...
            if chunk.is_final:
                ...
                if chunk.images:
                    _print_generated_images(chunk.images, image_output_dir)
